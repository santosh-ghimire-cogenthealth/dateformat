...
class DateController extends Controller
{
    ...
    {
        try{
            $diffNumber = $this->dateHelper->timeZoneComparasion($request->timezone1, $request->timezone2, $request->time, $request->format);
        }catch(Exception $e){
            return response(['error' => $e->getMessage(), ], 200);
        }
        return response(['timezone_compare' => $diffNumber, 'format' => !is_null($request->format) ? $request->format : 'Hours' ],200);
